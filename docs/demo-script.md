...
| 0:35–0:50 | **Visa wait (deterministic).** Show the estimate for an immediate relative. | **Measurable outcome:** estimator returns **0 years** (immediate relatives have no visa-bulletin wait) — computed from real Visa Bulletin data through **Dec 2025**, not the LLM. Source + date labeled. |
...
- **0-year** deterministic wait estimate for an immediate relative, from real
  Visa Bulletin data (Dec 2025) — reproducible, no model guesswork.
- **33/33 = 100%** on the deterministic eval (`npm run eval`): handoff
  precision/recall + estimator + no-legal-advice framing.
- **56 passing tests**, including handoff trigger coverage and **zero false
